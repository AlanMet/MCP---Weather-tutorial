...
// Options for RequestHandler constructor
...
class RequestHandler {
    ...
    constructor(options: RequestHandlerOptions = {}) {
        const {
            serverUrl = 'http://localhost:5000/v1/completions',
            model = 'user_data/models/openchat-3.5-0106.Q4_K_M.gguf',
            maxTokens = 1000,
            temperature = 0.2,
            stop = ["\nUser:", "\nAI:"],
            warmupPrompt = 'User: Hello!\nAI:'
        } = options;
        ...
        this.ready = false;
        this.fetchFn = fetch as unknown as Fetch; // will be assigned in create()
        this.errors = new Set();
    }
    ...
    private async _queryInternal(prompt: string): Promise<string> {
        const body = {
            model: this.model,
            prompt,
    ...
    async query(prompt: string, verbose = false): Promise<string> {
        if (!this.ready) {
